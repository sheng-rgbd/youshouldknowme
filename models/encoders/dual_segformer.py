...
from ..net_utils import FeatureFusionPHA as FFM  # ✅ 正確是 FeatureFusionPHA
...
class RGBXTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512], 
        ...
        #             FFM(dim=embed_dims[1], reduction=1, num_heads=num_heads[1], norm_layer=norm_fuse),  # stage 2
        ...
        self.FFMs = nn.ModuleList([
            FFM(dim=embed_dims[0], reduction=1, num_heads=num_heads[0], norm_layer=norm_fuse, input_resolution=(120, 160)),
            FFM(dim=embed_dims[1], reduction=1, num_heads=num_heads[1], norm_layer=norm_fuse, input_resolution=(60, 80)),
            FFM(dim=embed_dims[2], reduction=1, num_heads=num_heads[2], norm_layer=norm_fuse, input_resolution=(30, 40)),
            FFM(dim=embed_dims[3], reduction=1, num_heads=num_heads[3], norm_layer=norm_fuse, input_resolution=(15, 20)),
            # FFMPHA(dim=embed_dims[0], reduction=1, norm_layer=norm_fuse, input_resolution=(120, 160)),
            # FFMPHA(dim=embed_dims[1], reduction=1, norm_layer=norm_fuse, input_resolution=(60, 80)),
            # FFMPHA(dim=embed_dims[2], reduction=1, norm_layer=norm_fuse, input_resolution=(30, 40)),
            # FFMPHA(dim=embed_dims[3], reduction=1, norm_layer=norm_fuse, input_resolution=(15, 20)),
