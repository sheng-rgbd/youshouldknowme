...
# 先導入我們的 PHA module！
from models.decoders.pha_module import ParallelHybridAttention
...
class DeepLabV3Plus(nn.Module):
    def __init__(self, in_channels=[256, 512, 1024, 2048], num_classes=40, norm_layer=nn.BatchNorm2d):
        ...
        # PHA module 加入進來！
        self.pha = ParallelHybridAttention(in_channels=304)  # c4: 256 + c1: 48 = 304
    ...
    def forward(self, inputs):
        ...
        fusion = torch.cat([c4, c1], dim=1)
        fusion = self.pha(fusion)  # ✅ PHA 模組處理融合

        output = self.block(fusion)
        return output
